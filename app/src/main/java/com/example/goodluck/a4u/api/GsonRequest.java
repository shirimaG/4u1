package com.example.goodluck.a4u.api;
...
public class GsonRequest<T> extends Request<T> {
    /**
     * Default charset for JSON object
     */
     ...
     * @param requestBody     A {@link String} to post with request. Null is allowed and
     *                        indicate no parameters will be posted along with request
    ...
    protected Response<T> parseNetworkResponse(NetworkResponse response) {
        try {
            ...
            if (BuildConfig.DEBUG) {
                Timber.d("%s URL %s. ResponseCode: %s", this.getClass().getSimpleName(), requestUrl, response.statusCode);
            }
            // Parse response and return object
            String json = new String(response.data, PROTOCOL_CHARSET);
            // Parse json response to type <T>
    ...
    protected VolleyError parseNetworkError(VolleyError volleyError) {
        if (volleyError.networkResponse != null) {
            ...
            // If AccessToken expired. Logout user and redirect home page
            if (getStatusCode() == HttpURLConnection.HTTP_FORBIDDEN && fragmentManager != null) {
                // TODO: Implement login logout functionality
                Timber.e("Operation require authorization, status: %d ", requestStatusCode);
            } else {
                requestStatusCode = Constants.MISSING_STATUS_CODE;
            }
        }
